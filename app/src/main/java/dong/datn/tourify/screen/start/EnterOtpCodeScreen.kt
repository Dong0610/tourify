...
fun EnterOtpScreen(nav: NavController, viewModel: AppViewModel) {
    ...
    val titelMessage =
        context.getString(R.string.tt_mess_1) + viewModel.currentEmailVerify.value + context.getString(
            R.string.tt_mess_2
        )
    ViewParent(onBack = {
                ...
                Column(Modifier.fillMaxSize()) {
                    ...
                    Row(Modifier.fillMaxWidth()) {
                        InputValue(
                            ...
                            value = otp1.value, modifier = Modifier
                                .weight(1f)
                                .background(
                                    lightGrey, shape = RoundedCornerShape(8.dp)
                                ),
                            onValueChange = {
                                otp1.value = it
                        ...
                        InputValue(
                            keyboardType = KeyboardType.Number,
                            value = otp2.value, modifier = Modifier
                                .weight(1f)
                                .background(
                                    lightGrey, shape = RoundedCornerShape(8.dp)
                                ),
                        ...
                        InputValue(
                            keyboardType = KeyboardType.Number,
                            value = otp3.value, modifier = Modifier
                                .weight(1f)
                                .background(
                                    lightGrey, shape = RoundedCornerShape(8.dp)
                                ),
                    ...
                    AppButton(
                        ...
                        modifier = Modifier,
                        isEnable = otp1.value.isNotBlank() && otp2.value.isNotBlank() && otp3.value.isNotBlank() && otp4.value.isNotBlank()
                    ) {
                        val code = otp1.value + otp2.value + otp3.value + otp4.value
                        if (viewModel.otpCodeResponse.value!!.otpCode != "" && viewModel.otpCodeResponse.value!!.otpCode == code) {
                            nav.navigationTo(AccountScreen.ResetPassWordScreen.route)
                        }
                        else{
                            showToast(context.getString(R.string.otp_code_not_true))
                        }
                    }
                }
            }
